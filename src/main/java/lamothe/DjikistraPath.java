...
public class DjikistraPath {
    ...
    private static TilePos[][] shortedPath(TilePos[][] map, TilePos startPoint) {
        ...
        while(!meh.isEmpty()) {
            ...
            neighbours.add(new GameState.Position(u.getCurrentPos().getX() + 1, u.getCurrentPos().getY()));
            for(GameState.Position neighbour : neighbours) {
                // if not out of bounds
                if(neighbour.getX() >= 0 && neighbour.getX() < map.length && neighbour.getY() >= 0 && neighbour.getY() < map.length) {

                    String tileType = map[neighbour.getX()][neighbour.getY()].getCurrentTile().getSymbol();
                    //if (!tileType.equals(Tile.Wall)) {
                        int alt = dist[u.getCurrentPos().getX()][u.getCurrentPos().getY()] + getDistanceFromTile(map[neighbour.getX()][neighbour.getY()]);
                        if (alt < dist[neighbour.getX()][neighbour.getY()]) {
                            dist[neighbour.getX()][neighbour.getY()] = alt;
                            previous[neighbour.getX()][neighbour.getY()] = u;
                        }
                   // }
                }
            }
